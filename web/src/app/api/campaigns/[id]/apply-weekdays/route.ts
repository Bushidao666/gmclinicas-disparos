...
export async function POST(
  ...
  try {
    ...
    // Base de início: agora se startAt já passou
    const baseStart = startAt.getTime() < Date.now() ? new Date() : startAt;

    // Agendar: distribuir por 'dailyVolume' por dia permitido, iniciando em baseStart
    let currentDayStart = findNextAllowedDate(baseStart);
